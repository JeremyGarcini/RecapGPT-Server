...
// Recap pré-chargée avec timestamps simulés
const recapContext = {
  transcript:
  ...
  timestampedTranscript: `[0:00-0:08] MrBeast il est pas débile, il savait très bien qu'il y avait des gens qui allaient essayer de tricher
[0:08-0:18] en remplaçant par exemple un stylo pour tablette à la place d'un doigt qui pourrait rester sur l'écran toute la journée.
[0:18-0:25] Alors pour éviter ça, dans le concept du jeu, vous deviez très souvent bouger votre doigt.
[0:25-0:32] Sauf que ça n'a pas suffi. Des petits génies ont programmé des machines, des fois énormes,
[0:32-0:38] qui bougent des petits stylos tactiles. Vraiment, c'est une dinguerie ce qu'ils ont réussi à faire.
[0:38-0:45] Alors on sait pas pour autant si dans tous les gagnants, il y avait des tricheurs, mais on a quand même des doutes.
[0:45-0:55] D'ailleurs il y a un autre souci qui a tellement énervé les gens qu'en fait ils en avaient plus rien à foutre qu'il y ait des tricheurs sur l'application.
[0:55-1:02] En fait le vrai problème, c'est que l'appli il y avait tellement d'utilisateurs qu'elle était buggée.
[1:02-1:08] Et ça, ça a fait perdre beaucoup de gens. Si vous scrollez sur TikTok,
[1:08-1:18] vous pouvez peut-être voir passer des gens qui ont passé 32 heures sur l'application avec leur doigt collé, sans dormir,
[1:18-1:24] pour qu'il soit déconnecté par un bug. Toute la polémique que ça a créé,
[1:24-1:30] ça a amené une grosse remise en question dans la tête de MrBeast, qui a décidé de faire des changements.
[1:30-1:35] Mais ça, on va en reparler très vite. En parlant de glitch, on est clairement dans le thème
[1:35-1:42] d'un autre concours qu'a organisé MrBeast, cette fois-ci pour gagner 100 000 dollars sur le jeu le plus populaire du monde.`,
  summary:
...
${recapContext.timestampedTranscript}
...
async function startQuickChat() {
  console.log("🎬 RecapGPT - Quick Chat with Timestamps");
  console.log("📋 Recap loaded: TikTok video about MrBeast challenge problems");
  console.log("⏱️ Timestamps: Available (0:00 to 1:42)");
  console.log(
    "💡 Ask your questions! Try: 'Where did you hear about cheating?' (type 'quit' to exit)\n"
  );
